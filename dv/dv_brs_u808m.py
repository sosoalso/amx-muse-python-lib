...
class BrsU808m(CommonLogger, EventManager):
    ...
    def set_route(self, idx_in, idx_out):
        ...
        if idx_in < 1 or idx_in > 8 or idx_out < 1 or idx_out > 8:
            return
        if self.routes.get(idx_out, 0) == idx_in:
            return
        flag_in = idx_in - 1
    ...
    def set_routes(self, idx_in, idx_outs):
        ...
        # emit: routes(idx_in: int, idx_outs: list[int], routes: dict)
        self.emit("routes", idx_in=idx_in, idx_outs=idx_outs, routes=self.routes)
