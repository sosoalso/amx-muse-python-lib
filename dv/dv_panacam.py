...
class PanaCam:
    ...
    # ---------------------------------------------------------------------------- #
    def move_up(self):
        url_get(url=f"http://{self.ip_address}/cgi-bin/aw_ptz?cmd=%23T{50+self.get_speed():02d}&res=1", header={})

    def move_down(self):
        url_get(url=f"http://{self.ip_address}/cgi-bin/aw_ptz?cmd=%23T{50-self.get_speed():02d}&res=1", header={})

    def move_left(self):
        url_get(url=f"http://{self.ip_address}/cgi-bin/aw_ptz?cmd=%23P{50-self.get_speed():02d}&res=1", header={})
    ...
    def store_preset(self, preset_no):
        url_get(url=f"http://{self.ip_address}/cgi-bin/aw_ptz?cmd=%23M{preset_no:02d}&res=1", header={})
