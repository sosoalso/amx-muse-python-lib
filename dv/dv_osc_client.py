...
# # dv/dv_osc_client.py 실행 위치 기준으로 로컬 패키지 경로를 우선 탐색한다.
# _CUR_DIR = os.path.dirname(os.path.abspath(__file__))
# _ROOT_CANDIDATES = [
#     _CUR_DIR,
#     os.path.dirname(_CUR_DIR),
#     os.path.dirname(os.path.dirname(_CUR_DIR)),
#     os.path.dirname(os.path.dirname(os.path.dirname(_CUR_DIR))),
# ]
# _PATH_CANDIDATES = []
...
#         [
#             os.path.normpath(os.path.join(_root, "site-packages")),
#             os.path.normpath(os.path.join(_root, ".site-packages")),
#             os.path.normpath(os.path.join(_root, "lib", "site-packages")),
#             os.path.normpath(os.path.join(_root, "lib", "python3", "site-packages")),
#         ]
#     )
#     _PATH_CANDIDATES.extend(glob(os.path.join(_root, "lib", "python*", "site-packages")))

# for _candidate in _PATH_CANDIDATES:
#     if os.path.isdir(_candidate) and _candidate not in sys.path:
#         sys.path.insert(0, _candidate)

# # 마지막 보정: pythonosc 폴더를 직접 찾아 상위 폴더를 sys.path에 추가한다.
# for _root in _ROOT_CANDIDATES:
#     for _pattern in [
#         os.path.join(_root, "**", "pythonosc"),
#         os.path.join(_root, "**", "pythonosc", "osc_message.py"),
#     ]:
#         for _matched in glob(_pattern, recursive=True):
#             _pkg_dir = _matched if os.path.isdir(_matched) else os.path.dirname(_matched)
#             _pkg_parent = os.path.dirname(_pkg_dir)
#             if os.path.isdir(_pkg_dir) and _pkg_parent not in sys.path:
...
class OscClient(CommonLogger, EventManager):
    ...
    @handle_exception
    def init(self):
        self.dv.receive.listen(lambda evt: self.handle_receive(evt.arguments.get("data", b"")))
        self.dv.connect()
