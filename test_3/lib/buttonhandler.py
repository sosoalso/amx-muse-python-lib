        ...
        if evt.value:
            ...
            if self.hold_thread and self.hold_thread.is_alive() and threading.current_thread() != self.hold_thread:
                try:
                    ...
                    context.log.error(f"Error joining hold_thread: {e}")
                finally:
                    if self.hold_thread.is_alive():
                        context.log.warning("hold_thread did not terminate within the timeout.")
            if (
                self.repeat_thread
                ...
                try:
                    ...
                    context.log.error(f"Error joining repeat_thread: {e}")
                finally:
                    if self.repeat_thread.is_alive():
                        context.log.warning("repeat_thread did not terminate within the timeout.")
