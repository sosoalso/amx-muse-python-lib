...
def tp_add_notification(tp, port, button):
    ...
    # else:
    #     context.log.debug(f"tp_add_notification() 중복 알림 등록됨. 기존 알림을 유지합니다. {tp.id=} {port=} {button=}")


@handle_exception
def tp_add_notification_ss(tp_list, port, button):
    if not isinstance(tp_list, (list, tuple)):
        context.log.error("tp_add_notification_ss() 에러 : tp_list 는 장비로 튜플이나 리스트여야합니다.")
        raise TypeError
    for tp in tp_list:
...
def tp_add_notification_level_ss(tp_list, port, level):
    if not isinstance(tp_list, (list, tuple)):
        context.log.error("tp_add_notification_level_ss() 에러 : tp_list 는 장비로 튜플이나 리스트여야합니다.")
        raise TypeError
    for tp in tp_list:
...
def tp_add_watcher(tp, port, button, handler):
    ...
    if not tp.port[port].button[button].pythonWatchers or not handler in tp.port[port].button[button].pythonWatchers:
        ...
        context.log.debug(f"tp_add_watcher() 중복 등록됨. {tp.id=} {port=} {button=}")
...
def tp_add_watcher_ss(tp_list: list | tuple, port, button, handler):
    if not isinstance(tp_list, (list, tuple)):
        context.log.error("tp_add_watcher_ss() 에러 : tp_list 는 장비로 튜플이나 리스트여야합니다.")
        raise TypeError
    for tp in tp_list:
...
def tp_add_watcher_level_ss(tp_list: list | tuple, port, level, handler):
    if not isinstance(tp_list, (list, tuple)):
        context.log.error("tp_add_watcher_level_ss() 에러 : tp_list 는 장비로 튜플이나 리스트여야합니다.")
        raise TypeError
    for tp in tp_list:
...
def tp_get_button_state(tp, port, button):
    ...
    context.log.error(f"tp_get_button_state() : {tp.id} 은/는 온라인 상태가 아닙니다.")
    return False
...
def tp_set_button_ss(tp_list: list | tuple, port, button, value):
    if not isinstance(tp_list, (list, tuple)):
        context.log.error("tp_set_button_ss() 에러 : tp_list 는 장비로 튜플이나 리스트여야합니다.")
        raise TypeError
    for tp in tp_list:
        tp_set_button(tp, port, button, value)
...
def tp_set_button_in_array(tp, port, btn_list: list | tuple, index_condition):
    if not isinstance(btn_list, (list, tuple)):
        context.log.error("tp_set_button_in_array() 에러 : btn_list 는 버튼 인덱스 리스트여야합니다.")
        raise TypeError
...
def tp_set_button_in_array_ss(tp_list: list | tuple, port, btn_list: list | tuple, index_condition):
    if not isinstance(tp_list, (list, tuple)):
        context.log.error("tp_set_button_in_array_ss() 에러 : tp_list 는 장비로 튜플이나 리스트여야합니다.")
        raise TypeError
    for tp in tp_list:
        tp_set_button_in_array(tp, port, btn_list, index_condition)
...
def tp_set_button_in_range_ss(tp_list: list | tuple, port, index_btn_start, index_btn_range, index_condition):
    if not isinstance(tp_list, (list, tuple)):
        context.log.error("tp_send_level_ss() 에러 : tp_list 는 장비로 튜플이나 리스트여야합니다.")
        raise TypeError
    for tp in tp_list:
        tp_set_button_in_range(tp, port, index_btn_start, index_btn_range, index_condition)
...
def tp_get_level(tp, port, level):
    ...
    context.log.error(f"tp_get_level() : {tp.id} 은/는 온라인 상태가 아닙니다.")
    return 0
...
def tp_send_level_ss(tp_list: list | tuple, port, level, value):
    if not isinstance(tp_list, (list, tuple)):
        context.log.error("tp_send_level_ss() 에러 : tp_list는 튜플이나 리스트여야합니다.")
        raise TypeError
    for tp in tp_list:
        tp_send_level(tp, port, level, value)
...
def tp_send_command_ss(tp_list: list | tuple, port, command):
    if not isinstance(tp_list, (list, tuple)):
        context.log.error(f"tp_send_command_ss() {command=} 에러 : tp_list는 튜플이나 리스트여야합니다.")
        raise TypeError
    for tp in tp_list:
        tp_send_command(tp, port, command)
