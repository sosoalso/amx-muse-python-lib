...
def add_button(tp, port, channel, action, callback):
    ...
    context.log.debug(f"add_button() {tp=} 포트:{port} 채널:{channel} 액션:{action} 콜백:{callback}")
    return new_button
...
def add_button_ss(tp_list, port, channel, action, callback):
    ...
    context.log.debug(f"add_button_ss() {tp_list=} 포트:{port} 채널:{channel} 액션:{action} 콜백:{callback}")
    return new_button
...
def add_level_ss(tp_list, port, channel, callback):
    ...
    context.log.debug(f"add_level_ss() {tp_list=} 포트:{port} 채널:{channel} 콜백:{callback}")
    return level_handler
