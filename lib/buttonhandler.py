...
from lib.utility import debounce, run_thread, join_thread
...
class ButtonHandler(EventManager):
    def __init__(self, hold_time=30.0, repeat_interval=0.3, trigger_release_on_hold=False, init_action=None, init_handler=None):
        ...
        self._thread_hold: threading.Thread | None = None  # 홀드 감지 스레드
        self._thread_repeat: threading.Thread | None = None  # 반복 이벤트 스레드
        ...
        atexit.register(lambda: join_thread(self._thread_hold))
        atexit.register(lambda: join_thread(self._thread_repeat))
    ...
    def handle_event(self, evt):
        if evt.value:  # 버튼 눌림 (True)
            ...
            if "repeat" in self.actions and self.actions["repeat"]:
                run_thread(self._thread_repeat, self.start_repeat)
                # if self._thread_repeat is None or not self._thread_repeat.is_alive():
                #     self._thread_repeat = threading.Thread(target=self.start_repeat, daemon=True)
                #     self._thread_repeat.start()
            # hold 핸들러가 등록되어 있으면 홀드 감지 스레드 시작
            # 기존 스레드가 없거나 실행 중이 아닐 때만 새 스레드 생성 (중복 방지)
            if "hold" in self.actions and self.actions["hold"]:
                run_thread(self._thread_hold, self.start_hold)
                # if self._thread_hold is None or not self._thread_hold.is_alive():
                #     self._thread_hold = threading.Thread(target=self.start_hold, daemon=True)
                #     self._thread_hold.start()
        else:  # 버튼 뗌 (False)
